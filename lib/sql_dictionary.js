...
module.exports = {
   ...
   */
  findGoodsIdByCode: async (goodsCode) => {
    let _sql = `select id from t_goods where code = "${goodsCode}"`
    return sqlQuery.query(_sql)
  },
  /**  增加对应商品字典的 规格参数信息
   *  goods_id: 关联商品id
   ...
   *  net_weight: 净重量
   */
  insertGoodsSpecification: async (value) => {
    let _sql = 'insert into t_specification (create_time,goods_id,suitable_age,shelf_life,unit,related_parameters,specification,net_weight) values(now(),?,?,?,?,?,?,?);'
    return sqlQuery.query(_sql, value)
  },
}