...
module.exports = {
   ...
   *  goods_desc: 图文详情
   */
  updateGoodsById: async (value) => {
    let _sql = `update t_goods set edit_time = now(), code=?,title=?,abbr=?,brand_id=?,brand_name=?,category_id=?,category_name=?,unit_id=?,unit_name=?,sale_price=?,sort=?,goods_desc=? where id=?`
    return sqlQuery.query(_sql, value)
  },
  /** 通过id查找对应商品字典记录
   * @param  id:商品id
