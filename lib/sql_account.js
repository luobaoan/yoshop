...
module.exports = {
   ...
   * main_id：关联主账号 id
   */
  updateAccountMainId: async (value) => {
    let _sql = `update t_account set edit_time=now(),main_id=? where id=?`
    return sqlQuery.query(_sql, value)
  },
  /**
   * 添加注册用户资料
   ...
   *  username：用户名称
   *  sex：性别，默认 'secret'
   *  avatar：头像，默认 '/avatar.png'
   */
  insertUser: async (value) => {
    let _sql = 'insert into t_user(create_time,uid,username,sex,avatar) values(now(),?,"匿名","secret","/avatar.png")'
    return sqlQuery.query(_sql, value)
  },
